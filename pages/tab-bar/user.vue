<template>
  <view class="user">
    ...
    <view style="display: flex;justify-content: center;margin-top: 5%;padding-bottom: 10%">
      <u-button type="success" @click="hanldeDoctorUI">切换至医生端登录界面
      </u-button>
    </view>
    <u-tabbar
        v-model="current"
  ...
  methods: {
    hanldeDoctorUI() {
      getMall_doctor(this.$store.state.user.userId).then(re => {
        if (re.data == null) {
          this.navigateTo('/pages/tab-bar/doctor_apply/index')
        } else {
          if (re.data.status == 2) {
            this.showTip("审核未通过")
            return
            ...
            this.navigateTo('/pages/tab-bar/doctor_ui/index')
            this.showTip("进入医生界面")
            // this.navigateTo('/pages/tab-bar/doctor-ui/index')
          }
        }
      })
    },
