    ...
    <view class="shopSpecsPopup" :class="shopSpecsPopupStyleShow ? 'show' : 'hide'" @click.stop>
      ...
      <view class="content">
        ...
        <view class="number">
          ...
          <u-number-box width="200rpx" v-model="num" :min="selectSku.stock == 0 ? 0 : 1" :max="selectSku.id ? parseInt(selectSku.stock) : parseInt(showAreaStock[1])"></u-number-box>
        </view>
      ...
      <view class="btnBox">
        <view class="confirm" :style="{
						background: selectSku.id && selectSku.stock * 1 < 1 ? '#b4b4b4' : 'linear-gradient(to right, #ffd01e, #ff8917)'
					}" @click="addCart">
        ...
        <view class="confirm" :style="{
						background: selectSku.id && selectSku.stock * 1 < 1 ? '#b4b4b4' : 'linear-gradient(to right, #ff6034, #ee0a24)'
					}" @click="confirm">
          <text class="confirm-text">
            立即购买
          </text>
...
<script>
// 引入 CryptoJS 库
import CryptoJS from "@/utils/CryptoJS.min";
import mpMixin from '@/components/wu-ui-tools/libs/mixin/mpMixin.js';
import mixin from '@/components/wu-ui-tools/libs/mixin/mixin.js';
 ...
 * @event {Function} addCart 点击确认按钮时触发的事件，会返回e， e = { sku, skuText , num }，分别对应选中的sku值 、sku属性名 、输入框内的数量。
...
@import '@/components/wu-ui-tools/libs/css/common.scss';
@import '@/components/wu-ui-tools/libs/css/components.scss';
@import '@/components/wu-ui-tools/libs/css/color.scss';
