...
<script>
// 引入绘制插件
import DrawPoster from "@/components/u-draw-poster";
...
export default {
  data: () => ({
    ...
    logo: require("@/static/images/logo.png"), //本地logo地址
  }),
  ...
  methods: {
    ...
    async draw(dp) {
      const { width, height, background, title } = this.shareInfo.container;
      const { code, img, price } = this.shareInfo.bottom;
